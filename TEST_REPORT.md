...
**Container:** libvips (UltraHDR enabled) + pyvips
**Upstreams used for this build:** `LIBVIPS_REF=master`, `LIBUHDR_REF=main`
**pyvips:** 3.0.0 (from container)

> Goal: validate read/write behavior for UltraHDR (JPEG gain map) when **compressing, resizing, and cropping**. We test both **P3** and **sRGB** source images (square 1080×1080).

---

## Reference images (click to open)

* [Display P3 sample](samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg)
* [sRGB sample](samples/ISO_JPG_sRGB_transcoding_test_Greg_Benz.jpg)

Extracted profiles:

* [P3 ICC profile](samples/ISO_JPG_P3_transcoding_test_Greg_Benz_profile.icc)
* [sRGB ICC profile](samples/ISO_JPG_sRGB_transcoding_test_Greg_Benz_profile.icc)
...
docker compose -f docker/compose.yml run --rm pyvips \
  ...
  /work/samples/ISO_JPG_sRGB_transcoding_test_Greg_Benz_profile.icc
```
...
* [P3\_q75.jpg](samples/P3_q75.jpg)
* [P3\_q75\_profile.icc](samples/P3_q75_profile.icc)
...
* [P3\_w512.jpg](samples/P3_w512.jpg)
* [P3\_w512\_profile.icc](samples/P3_w512_profile.icc)

**Takeaway:** Base was resized, but the embedded gain map remained at 1080×1080 (current behavior of our simple script). Visual HDR looked correct in quick checks, but for production you may want to sync the map if your renderer expects matched geometry.

---

## 4) Crop

### 4A) Crop **without** syncing the map (intentional mismatch)

```bash
docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_ops.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_crop_640_nosync.jpg \
  --crop 220 220 640 640 -q 85
```
...
* [P3\_crop\_640\_nosync.jpg](samples/P3_crop_640_nosync.jpg)
* [P3\_crop\_640\_nosync\_profile.icc](samples/P3_crop_640_nosync_profile.icc)
...
```bash
docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_crop_sync.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_crop_640_sync.jpg \
  --crop 220 220 640 640 -q 85
```
...
* [P3\_crop\_640\_sync.jpg](samples/P3_crop_640_sync.jpg)
* [P3\_crop\_640\_sync\_profile.icc](samples/P3_crop_640_sync_profile.icc)
...
docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_ops.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_q75.jpg -q 75

docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_ops.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_w512.jpg --width 512 -q 85

docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_ops.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_crop_640_nosync.jpg --crop 220 220 640 640 -q 85

docker compose -f docker/compose.yml run --rm pyvips \
  /work/scripts/ultrahdr_crop_sync.py \
  /work/samples/ISO_JPG_P3_transcoding_test_Greg_Benz.jpg \
  /work/samples/P3_crop_640_sync.jpg --crop 220 220 640 640 -q 85
```
